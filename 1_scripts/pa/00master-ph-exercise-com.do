...
#2 Stata Settings
#3 Defines globals
#4 Create working directories & define globals (physical activity
...
if "`c(username)'" == "[YOUR USER NAME]" {
	global rdta "[PATH WHERE REPO IS SAVED]"
}
...
do "$code/01cr-hilda$ps.do"    		// Extracts physical activity variables from HILDA
do "$code/02cr-persyr-psid$ps.do"   // Extracts parenthood variables from PSID
do "$code/03cr-shp$ps.do"    		// Extracts physical activity variables from SHP
do "$code/04cr-soep$ps.do"    		// Extracts physical activity variables from SOEP
do "$code/05cr-andata$ps.do"    	// Combines single country data sets
do "$code/06an-analysis$ps.do"		// Descriptive & treatment effect analysis
